...
nifti_dir=$(pwd)/niftis

# Output directory setup
output_dir=$(pwd)/ROI_analysis
mkdir -p $output_dir
...
for i in "${!locations[@]}"; do
  ...
  for volume_file in "$nifti_dir"/*.nii*; do
    ...
    if [ -z "$mean_value" ]; then
      ...
      echo "Error extracting mean value for ${volume_file} at ${location}"
      continue
    fi

    echo "Extracted mean value: $mean_value"
    echo "Extracted max value: $max_value"
