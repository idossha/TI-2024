...
# seem to work well but tensors orientation is probably wrong
...
for subj in $(cat subjList.txt); do
    ...
    if [ -d "${subj}/dwi" ]; then
        ...
        if [ -f "${subj}_dwi_reoriented.nii.gz" ] && [ -f "${subj}_dwi.bvec" ] && [ -f "${subj}_dwi.bval" ]; then
            echo "Converting NIFTI to MIF format..."
            mrconvert -fslgrad ${subj}_dwi.bvec ${subj}_dwi.bval ${subj}_dwi_reoriented.nii.gz ${subj}.mif -force
            if [ ! -f "${subj}.mif" ]; then
            ...
            if [ -f "eddy_corrected.nii.gz" ] && [ -f "${subj}_dwi.bvec" ] && [ -f "${subj}_dwi.bval" ]; then
                echo "Converting preprocessed file to NIFTI format with gradient information..."
                mrconvert -fslgrad ${subj}_dwi.bvec ${subj}_dwi.bval eddy_corrected.nii.gz ${subj}_corrected.nii.gz -force
            ...
            dtifit --data=${subj}_corrected.nii.gz --out=dti --mask=${subj}_corrected_bet.nii.gz --bvecs=${subj}_dwi.bvec --bvals=${subj}_dwi.bval
        ...
        echo "Directory ${subj}/dwi not found, skipping subject."
    fi
done
